...
import {
	createDocumentStore,
	useDocumentStore,
} from "../../src/store/document-store.ts"
...
describe("useDocumentStore", () => {
	...
	it("queues changes until the handle is ready", async () => {
		const {create, wrapper} = setup()
		const handleSlow = create()
		handleSlow.change(doc => (doc.key = "slow"))
		const oldDoc = handleSlow.doc.bind(handleSlow)
		...
		return testEffect(async done => {
			createEffect((run: number = 0) => {
				if (run == 0) {
					...
					expect(doc()?.key).toBe("early")
					done()
				}
				return run + 1
			})
			change(doc => (doc.key = "this"))
			change(doc => (doc.key = "is"))
			change(doc => (doc.key = "early"))
		})
	})
